...
from networks.detr_3d.detector_Vote2Cap_DETR.helpers import (
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                dec_cast: nn.ModuleList = None, 
                frame_feature: torch.Tensor = None, 
        ...
        for idx, layer in enumerate(self.layers):
            if (dec_cast is None) or (frame_feature is None):
                output, attn = layer(output, memory, tgt_mask=tgt_mask,
                           ...
                           cast_module=None, frame_feature=None, frame_mask=None)
            else:
                output, attn = layer(output, memory, tgt_mask=tgt_mask,
                           memory_mask=memory_mask,
                           tgt_key_padding_mask=tgt_key_padding_mask,
                           ...
                           pos=pos, query_pos=query_pos,
                           return_attn_weights=return_attn_weights,
                           cast_module=dec_cast[idx], frame_feature=frame_feature, frame_mask=frame_mask)
...
class MaskedTransformerEncoder(TransformerEncoder):
    ...
    def forward(self, src,
                ...
                xyz: Optional [Tensor] = None,
                transpose_swap: Optional[bool] = False,
                enc_cast: Optional[nn.ModuleList] = None,
                frame_feature: Optional[torch.Tensor] = None,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.masking_radius[idx] > 0:
                ...
                mask = mask.repeat(1, nhead, 1, 1)
                mask = mask.view(bsz * nhead, n, n)
            if (enc_cast is None) or (frame_feature is None):
                output = layer(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, pos=pos, cast_module=None, frame_feature=None, frame_mask=None)
            else:
                output = layer(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, pos=pos, cast_module=enc_cast[idx], frame_feature=frame_feature, frame_mask=frame_mask)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
                     ...
                     src_key_padding_mask: Optional[Tensor] = None,
                     pos: Optional[Tensor] = None, 
                     cast_module: Optional[nn.ModuleDict] = None,
                     frame_feature: Optional[torch.Tensor] = None,
                     frame_mask: Optional[torch.Tensor] = None):
        ...
        # Add adapter after self-attention, before residual connection
        if cast_module is not None:
            src2 = cast_module["pre_adapter"](src2.permute(1, 0, 2)).permute(1, 0, 2)  # src2: [len, bs, d]
        src = src + self.dropout1(src2)
        
        # Add uni-cast before FFN
        if cast_module is not None:
            src = src + cast_module["cast"](pc_token=src.permute(1, 0, 2), frame_token=frame_feature, frame_token_mask=frame_mask.bool()).permute(1, 0, 2)  # src2: [len, bs, d]
        ...
        if self.use_ffn:
            # Add adapter in FFN
            if cast_module is not None:
                src3 = cast_module["post_adapter"](src2.permute(1, 0, 2)).permute(1, 0, 2)  # src3s: [len, bs, d]
            
            src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
            
            # Add residual
            if cast_module is not None:
                src = src + self.dropout2(src2) + src3
            else:
    ...
    def forward(self, src,
        ...
        if self.normalize_before:
            return self.forward_pre(src, src_mask, src_key_padding_mask, pos, return_attn_weights, cast_module, frame_feature, frame_mask)
        return self.forward_post(src, src_mask, src_key_padding_mask, pos, cast_module, frame_feature, frame_mask)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
                     ...
                     query_pos: Optional[Tensor] = None,
                     return_attn_weights: Optional [bool] = False,
                     cast_module: nn.ModuleDict = None, 
                     frame_feature: torch.Tensor = None, 
                     frame_mask: torch.Tensor = None):
        ...
        # Add adapter after self&cross-attention, before residual connection
        if cast_module is not None:
            tgt2 = cast_module["pre_adapter"](tgt2.permute(1, 0, 2)).permute(1, 0, 2)  # tgt2: [len, bs, d]
        tgt = tgt + self.dropout2(tgt2)
        
        # Add uni-cast before FFN
        if cast_module is not None:
            tgt = tgt + cast_module["cast"](pc_token=tgt.permute(1, 0, 2), frame_token=frame_feature, frame_token_mask=frame_mask.bool()).permute(1, 0, 2)  # tgt: [len, bs, d]
        
        tgt = self.norm2(tgt)
        
        # Add adapter in FFN
        if cast_module is not None:
            tgt3 = cast_module["post_adapter"](tgt.permute(1, 0, 2)).permute(1, 0, 2)  # tgt3s: [len, bs, d]
        
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
        
        if cast_module is not None:
            tgt = tgt + self.dropout3(tgt2) + tgt3
        else:
    ...
    def forward(self, tgt, memory,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_mask, memory_mask,
                                    ...
                                    cast_module=cast_module, frame_feature=frame_feature, frame_mask=frame_mask)
